...
async function refreshContest() {
    ...
    try {
        ...
        const contestFeedChannel = await client.channels.fetch(process.env['CONTEST_FEED_CHANNEL_ID']);
            ...
            if(userParticipations.get(ranking[i].user) == undefined) {
                ...
                for(let j = 0; j < numProblems; j++) {
                    ...
                    if(newScore == Contest.problems[j].points) {
                        ...
                        await sendMessage(channelBotFeed, messageStr, (ranking[i].old_rating >= 2400)||(j+1 >= 4), 1);
                    } else {
                    ...
                    if(oldInfo.solutions[j] == null && ranking[i].solutions[j] != null) {// new attempt
                        if((j+1) >= 4) {// send message for P4-P6
    ...
    setTimeout(refreshContest, 6000);
}
